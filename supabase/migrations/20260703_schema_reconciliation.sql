-- Schema reconciliation: the live database had drifted from the migration
-- history. This adds the missing shifts.location column and the three tables
-- that were never applied (user_private, banking_details, payout_*).
-- Fully idempotent — safe to run more than once.

-- ── shifts.location (post-shift was failing: column missing) ────────────────
alter table public.shifts
  add column if not exists location text not null default 'Kuala Lumpur';

-- ── user_private (owner-only identity PII written at registration) ──────────
...
on public.payout_item for select using (auth.uid() = worker_id);

drop policy if exists payout_item_employer_read on public.payout_item;
create policy payout_item_employer_read
on public.payout_item for select using (auth.uid() = employer_id);
...
on public.payout_cycle for select using (auth.role() = 'authenticated');
...
on public.payout_audit for select using (auth.role() = 'authenticated');
