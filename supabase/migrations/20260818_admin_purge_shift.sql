...
--
-- THIRD ATTEMPT, and the previous two are why this one is simple.
--   1st: ended with `delete from public.shifts`, which
--        guard_delete_of_booked_shift blocks for any shift with confirmed
--        workers -- and 20260814 deliberately does not exempt admins. I wrote
--        that non-exemption myself and forgot it two days later.
--   2nd: taught the guard to honour a transaction-local bypass flag. Still
--        failed with the guard reporting from its OLD line 4, i.e. the
--        replacement definition was not the one running.
--
-- So this attempt stops trying to get around the guard. The guard's only test
-- is "does any application on this shift have status = 'accepted'". Deleting
-- those applications FIRST satisfies it honestly, and the shift delete then
-- passes on its own merits.
--
-- That is better than a bypass regardless of which approach would have
-- worked: the guard keeps zero exceptions, there is no flag that could leak
-- or be set by something else later, and the guard function is not touched at
-- all -- so nothing about employer-facing behaviour changes.
--
...
-- permanently undeletable by ANYONE, admin included -- this project's own
-- verification needed hand-written SQL from the owner four separate times.
-- That is a missing capability, not a safety feature.
--
-- SAFETY. A hard delete with no undo, so it is bounded:
--   * admin-only, checked inside the function rather than delegated to RLS;
...
--   * a reason is required and logged before the rows go.
...
begin
  ...
  -- Logged before anything is removed, so the record survives a rollback.
  raise warning 'admin_purge_shift: % ("%") by % -- % application(s) -- reason: %',
    p_shift_id, coalesce(v_shift.title, '?'), auth.uid(), v_apps, p_reason;

  -- Notifications have no FK to either table, so they never cascade. Removed
  -- explicitly, or the purge leaves exactly the dangling links the tombstone
  -- rendering exists to explain.
  ...
  -- Applications FIRST. This is the whole trick: guard_delete_of_booked_shift
  -- refuses while any application here is 'accepted', so removing them makes
  -- the shift delete below legitimate rather than exempt.
...
-- ── one-off: clear the QA rows this project's verification stranded ──────────
-- Same order as the function, for the same reason.
delete from public.notifications
where link in (
  select '/worker/applications/' || a.id::text
